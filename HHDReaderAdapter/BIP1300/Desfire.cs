   ...
   public class Desfire : ISO14443
    {
       ...
       /// <returns></returns>
        public bool SelectCardApplication(byte nAID)
        {
            ...
            byte[] m_response;
            ret = base.ExchangeAPDU((byte)cmd_selectApp.Length, cmd_selectApp, out m_response);
       ...
       /// <returns></returns>
        public bool WriteDataFile(byte nFileID,int nOffset, int nDataLen, byte[] abyWriteData)
        {
            bool bRet = false;
            byte[] m_response;
            int index = 0;
            ...
            bRet = base.ExchangeAPDU((byte)cmd_buff.Length, cmd_buff, out m_response);
             ...
             if (bRet)
             {
                 if (m_response[0] == 0x00) //Data written successfully
                 {

                 }
                 else if (m_response[0] == 0xAF) // more data to be written
                 {
                     ...
                     bRet = base.ExchangeAPDU((byte)cmd_buff2.Length, cmd_buff2, out m_response);
                     if (bRet)
                     {
                         if (m_response[0] == 0x00) //Data written successfully
                         {
       ...
       /// <returns></returns>
        private bool ReadRecordIntermideate(ref byte[] mRecordBuff, int index)
        {
            ...
            if (bRet)
            {
                ...
                else if (pSw1 == 0x91 && pSw2 == 0xAF) //continure to read
                {
                    Array.Copy(m_response, 0, mRecordBuff, index, nbRead);
                    index += (nbRead);
                    ReadRecordIntermideate(ref mRecordBuff, index);
                }
                else
                {
                    bRet = false;
        ...
        /// <param name="abyWriteData"></param>
        /// <returns></returns>
        public bool WriteRecords(byte nFileID,int nOffset, int nDataLen, byte[] abyWriteData)
        {
            bool bRet = false;
            int index = 0;
            byte[] m_response;
            ...
            bRet = base.ExchangeAPDU((byte)cmd_buff.Length, cmd_buff, out m_response, out nbyteRead, out pSw1, out pSw2);
              ...
              if (bRet)
              {
                  ...
                  {
                    bRet=  WriteRecordsIntermediate(ref abyWriteData, 52, nDataLen - MaxDatatobeSent);
                  }
                  else
                  {
                      bRet = false;
